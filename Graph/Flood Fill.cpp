...
void dfs(vector<vector<int>> &arr, int r, int c, int m, int n, int oldCol, int newCol) {
    if(r < 0 || r >= m || c < 0 || c >= n || arr[r][c] != oldCol) return;
    arr[r][c] = newCol;
    dfs(arr, r+1, c, m, n, oldCol, newCol);
    dfs(arr, r-1, c, m, n, oldCol, newCol);
    dfs(arr, r, c+1, m, n, oldCol, newCol);
    dfs(arr, r, c-1, m, n, oldCol, newCol);
}
...
void solve() {
	...
	rep(i, 0, m) {rep(j, 0, n) {cin >> image[i][j]}};

	int newCol, oldCol, r, c;
	cin >> r >> c;
	oldCol = image[r][c];
	cin >> newCol;

	if(oldColor == newColor) return image;
    dfs(image, r, c, m, n, oldCol, newCol);
    rep(i, 0, m) {rep(j, 0, n) {cout << image[i][j]}};
}
