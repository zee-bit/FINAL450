...
ll solveWW(int idx, int k, vector<int> &nums, vector<ll> &dp, int prev_cost) {
    int N = nums.size();
    if(idx == N) return (-1 * prev_cost);
    if(dp[idx] != -1) return dp[idx];
    
    ll prev = 0, ans = INT_MAX;
    for(int i = idx; i < N; i++)
    {
        prev += nums[i];
        if(prev > k) break;
        ...
        ll temp = x + solveWW(i + 1, k, nums, dp, x);
