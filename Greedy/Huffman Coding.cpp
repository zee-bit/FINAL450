...
void preorder(Node *node, string curr, vector<string> &res) {
	if(!node) return;
    if(node -> left == NULL && node -> right == NULL) {
        res.push_back(curr);
        return;
    }
    if(node -> left) preorder(node -> left, curr + "0", res);
    if(node -> right) preorder(node -> right, curr + "1", res);
}
