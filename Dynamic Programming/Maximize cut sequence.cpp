...
void solve() {
    ...
    for(int i = 1; i <= n; i++) {
        int x_part = (i-x >= 0 ? dp[i-x] : 0);
        int y_part = (i-y >= 0 ? dp[i-y] : 0);
        int z_part = (i-z >= 0 ? dp[i-z] : 0);
        if(x_part == 0 and y_part == 0 and z_part == 0) continue;
        dp[i] = 1 + max({x_part, y_part, z_part});
    }
    cout << dp[n];
}
