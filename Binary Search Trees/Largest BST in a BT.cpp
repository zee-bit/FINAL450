...
vector<int> getLargestBST(Node *node) {
    if(!node)
        return {INT_MIN, INT_MAX, 0};
    
    vector<int> left = getLargestBST(node -> left);
    vector<int> right = getLargestBST(node -> right);
    
    if(left[0] < node -> data && right[1] > node -> data)
        return {max(right[0], node -> data), min(left[1], node -> data), 1 + left[2] + right[2]};
    else 
...
void solve() {
	Node *root = buildTree();
	vector<int> state = getLargestBST(root);
    cout << state[2];
}
