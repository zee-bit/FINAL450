...
void solve() {
    ...
    string first = "", second = "";
    while(!pq.empty()) {
        if(pq.top() == 0) { pq.pop(); continue; }
        first = char(pq.top() + '0') + first;
        pq.pop();
        if(!pq.empty()) {
            if(pq.top() == 0) { pq.pop(); continue; }
            second = char(pq.top() + '0') + second;
            pq.pop();
        }
    }
    // cout << first << " " << second << "\n";
    string res = "";
    int i = 0, j = 0, carry = 0, F = first.size(), S = second.size();
    
    while(i < F && j < S) {
        int sm = carry + (first[i] - '0') + (second[j] - '0');
        res = char(sm % 10 + '0') + res;
        carry = sm / 10;
        i++; j++;
    }
    while(i < F) {
        int sm = carry + (first[i] - '0');
        res = char(sm % 10 + '0') + res;
        carry = sm / 10;
        i++;
    }
    while(j < S) {
        int sm = carry + (second[j] - '0');
        res = char(sm % 10 + '0') + res;
        carry = sm / 10;
        j++;
    }
