...
void solve() {
    ...
    while(true) {
        ...
        int num = top.first, ar_id = top.second.first, idx = top.second.second+1;
        ...
        if(idx == n) break;
        high = max(high, KSortedArray[ar_id][idx]);
        pq.push({KSortedArray[ar_id][idx], {ar_id, idx}});
    }
