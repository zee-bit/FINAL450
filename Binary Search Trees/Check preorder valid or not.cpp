...
Node *postorder(vector<int> &pre, int start, int end) {
    if(start > end) return NULL;
    
    Node *node = newNode(pre[start]);
    int mid = start + 1;
    while(mid <= end && pre[mid] < pre[start]) mid++;
    
    node -> left = postorder(pre, start + 1, mid - 1);
    node -> right = postorder(pre, mid, end);
