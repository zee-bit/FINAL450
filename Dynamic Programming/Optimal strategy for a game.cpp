...
ll maxMoney(int i, int j, vector<int> &arr, vector<vector<ll>> &dp) {
	if(i > j) return 0;
    if(i == j) return arr[i];
    
    if(dp[i][j] != -1) return dp[i][j];
    
    int x = arr[i] + min(maxMoney(i+2, j, arr, dp), maxMoney(i+1, j-1, arr, dp));
    int y = arr[j] + min(maxMoney(i+1, j-1, arr, dp), maxMoney(i, j-2, arr, dp));
