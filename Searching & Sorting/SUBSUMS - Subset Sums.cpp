...
void getSums(vll &arr, ll lo, ll hi, vll &sums) {
	ll n = hi - lo + 1, p = 1 << n;
	rep(i, 0, p) {
		ll sum = 0;
		rep(j, 0, n) {
...
void solve() {
	...
	// partition the array into half and solve for each half
	vll left, right;
	getSums(arr, 0, N/2, left);
	getSums(arr, N/2+1, N-1, right);

	// sort the second array so that we can BS
	sort(all(right));

	// for each sum from 1st set, find the sum in 2nd set whose sum lies in [A,B]
	ll cnt = 0;
	rep(i, 0, left.size()) {
		int low = lower_bound(all(right), A-left[i]) - right.begin();
		int high = upper_bound(all(right), B-left[i]) - right.begin();
		cnt += (high - low);
	}
	cout << cnt;
}
