...
void fillInOrder(Node *node, vector<int> &arr) {
	if(!node) return;
	fillInOrder(node -> left, arr);
	arr.push_back(node -> data);
	fillInOrder(node -> right, arr);
}

void freeInOrder(Node *node) {
	if(!node) return;
	freeInOrder(node -> left);
	free(node);
	freeInOrder(node -> right);
}

void toMinHeap(Node *node, int &idx, vector<int> &arr) {
	if(!node) return;
	node -> data = arr[idx++];
	toMinHeap(node -> left, idx, arr);
	toMinHeap(node -> right, idx, arr);
}
