...
int solve() {
	int n;
	cin >> n;
	vi petrol(n), distance(n);
	rep(i, 0, n) cin >> petrol[i];
	rep(i, 0, n) cin >> distance[i];

	int extra_fuel = 0, total_extra_fuel = 0, start = 0;
        
    for(int i = 0; i < n; i++) {
        total_extra_fuel += petrol[i] - distance[i];
    ...
    if(total_extra_fuel < 0) return -1;
    return start;
}
