...
void fillSet(Node *node, unordered_set<int> &st) {
    if(!node) return;
    fillSet(node -> left, st);
    st.insert(node -> data);
    fillSet(node -> right, st);
}

void countPairs(Node *node, unordered_set<int> &st, int &x, int &cnt) {
    if(!node) return;
    countPairs(node -> left, st, x, cnt);
    if(st.count(x - node -> data)) cnt++;
    countPairs(node -> right, st, x, cnt);
}
