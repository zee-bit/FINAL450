...
int liss(Node *node) {
	...
	int lis_excl = liss(node -> left) + liss(node -> right);
	int lis_incl = 1;
	if(node -> left)
		lis_incl += liss(node -> left -> left) + liss(node -> left -> right);
	if(node -> right)
		lis_incl += liss(node -> right -> left) + liss(node -> right -> right);

	node -> lis = max(lis_incl, lis_excl);
	return node -> lis;
}
