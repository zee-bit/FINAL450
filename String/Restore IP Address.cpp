...
void restore(string &s, int idx, int N, int block, string curr, vector<string> &res) {
    if(idx == N && block == 5) {
    ...
    for(int i = idx; i < idx + 3 && i < N; i++) {
        ...
        // If block starts with '0' and is not 0
        if(temp[0] == '0') {
            if(num_temp != 0 || temp.size() > 1)
                return;
        }
        if(num_temp > 255)
            return;
        
        restore(s, i + 1, N, block + 1, curr + '.' + temp, res);
    }
}
