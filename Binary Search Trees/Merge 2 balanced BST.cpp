...
void getNodes(TreeNode *node, vector<int> &nodes) {
    if(!node) return;
...
void solve() {
	...
	vector<int> tree1;
	getNodes(root1, tree1);
	
	vector<int> tree2;
	getNodes(root2, tree2);

	int M = tree1.size(), N = tree2.size();
	vector<int> merged = merge(tree1, tree2, M, N);

	Node *newRoot = buildTree(merged);
}
