...
int minMoves(int e, int f, vector<vector<int>> &dp) {
    if(e == 1 || f <= 1) return f;
    ...
    for(int k = 1; k <= f; k++) {
        int broke = minMoves(e-1, k-1, dp);
        int safe = minMoves(e, f-k, dp);
        
        int worst = 1 + max(broke, safe);
        moves = min(moves, worst);
    }
    ...
    while(lo <= hi) {
        ...
        int down = minMoves(e-1, mid-1, dp);
        int up = minMoves(e, f-mid, dp);
        int worst = 1 + max(up, down);
