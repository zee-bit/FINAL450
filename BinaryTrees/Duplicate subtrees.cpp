...
string getDuplicates(Node *node, vector<Node*> &res, unordered_map<string, int> &mp) {
    if(!node) return "$";
    
    string str = to_string(node -> data) + ".";
    str += getDuplicates(node -> left, res, mp) + ".";
    str += getDuplicates(node -> right, res, mp);
...
void solve() {
    ...
    getDuplicates(root, res, mp);

    rep(i, 0, res.size()) printTree(res[i]);
}
