...
struct DoublyLinkedList {
    ...
    vector<vector<int>> findSum(int sum) {
        vector<vector<int>> res;
        Node *first, *second = head -> next, *third = tail;
        
        for(first = head; first -> next; first = first -> next) {
            int target = sum - first -> val;
            second = first -> next, third = tail;

            while(second != third && second -> prev != third) {
                int curr_sum = second -> val + third -> val;
                if(curr_sum > target)
                    third = third -> prev;
                else if(curr_sum < target)
                    second = second -> next;
                else {
                    res.push_back({first -> val, second -> val, third -> val});
                    second = second -> next;
                    third = third -> prev;
                }
            }
