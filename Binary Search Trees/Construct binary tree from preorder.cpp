...
Node *buildTree(vector<int> &preorder, int &idx, int high) {
    if(idx == preorder.size() || preorder[idx] >= high)
    	return NULL;
    
    Node *node = new Node(preorder[idx++]);
    node -> left = buildTree(preorder, idx, node -> val);
    node -> right = buildTree(preorder, idx, high);
    return node;
}
...
void solve() {
	...
	int idx = 0;
	Node *root = buildTree(preorder, idx, INT_MAX);
	cout << root -> data;
}
