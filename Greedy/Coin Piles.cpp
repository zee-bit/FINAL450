...
void solve() {
    ...
    for(int i = 0; i < N; i++) {
        ...
        int high = A[i] + K;
        int idx = upper_bound(all(A), high) - A.begin();
        int currSum = prefSum[N - 1] - prefSum[idx - 1];
        int reqSum = (N - idx) * high;
        int excess = currSum - reqSum;
        res = min(res, excess + prevSum);
        prevSum += (A[i] * cnt);
    }
