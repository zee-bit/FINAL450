...
void solve() {
	...
	// Preprocess last row
	for(int j = N-2; j >= 0; j--)
		maxArr[N-1][j] = max(maxArr[N-1][j+1], arr[N-1][j]);
	
	// Preprocess last column
	for(int i = N-2; i >= 0; i--)
		maxArr[i][N-1] = max(maxArr[i+1][N-1], arr[i][N-1]);

	// Preprocess the rest of matrix from bottom to top
	for(int i = N-2; i >= 0; i--) {
		for(int j = N-2; j >= 0; j--) {

			maxVal = max(maxVal, maxArr[i+1][j+1] - arr[i][j]);
			maxArr[i][j] = max({arr[i][j], maxArr[i][j+1], maxArr[i+1][j]});
		}
	}
	cout << maxVal;
}
