...
int solve() {
	...
	int lo = 0, hi = N - 1, mid, min_idx = 0;
    
    // METHOD-1
    // 1. Find the index of minimum element (Inflection point)
    while(lo <= hi) {
        mid = lo + (hi - lo) / 2;
        if(nums[mid] < nums[0]) {
            min_idx = mid;
    ...
    // 2. Check in which part our target lies
    lo = 0, hi = N - 1;
    ...
    // 3. Perform normal BS to find the target element;
    while(lo <= hi) {
        mid = lo + (hi - lo) / 2;
        if(nums[mid] == target) 
            return mid;
