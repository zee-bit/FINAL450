...
int subtreeSum(Node* node) {
    ...
    if(node -> left) {
        ll = subtreeSum(node -> left -> left);
        lr = subtreeSum(node -> left -> right);
    }
    if(node -> right) {
        rl = subtreeSum(node -> right -> left);
        rr = subtreeSum(node -> right -> right);
    }

    dp[node] = max(left+right, node -> val + ll+lr+rl+rr);
    return dp[node];
}
