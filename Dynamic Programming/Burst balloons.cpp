...
int maxCoins(int left, int right, vector<int> &arr, vector<vector<int>> &dp) {
	...
	for(int k = left + 1; k < right; k++) {
		cost = max(cost, arr[left] * arr[k] * arr[right] + maxCoins(left, k, arr, dp) + maxCoins(k, right, arr, dp));
	}
